...
def load_docvqa_test_data(test_data_path):
    """
    加载DocVQA测试数据，格式与LayTextLLM项目一致
    """
    print(f"正在加载DocVQA测试数据: {test_data_path}")
...
def main():
    ...
    image_folder_path = os.environ.get(
        "EVAL_IMAGE_FOLDER", os.path.join(_REPO_ROOT, "dataset/DocVQA/images"))

    output_base_path = os.environ.get(
        "EVAL_OUTPUT_BASE",
        os.path.join(_REPO_ROOT, "eval/result/docvqa/"),
    )  # 输出根路径

    docvqa_test_data_path = os.environ.get(
        "EVAL_DATA_PATH", os.path.join(_REPO_ROOT, "dataset/DocVQA/docvqa_test.json"))
    ...
    # 加载DocVQA测试数据
    docvqa_data = load_docvqa_test_data(docvqa_test_data_path)
    
    # ==================== [CHECKPOINTING LOGIC - START] ====================

    # <--- 新增: 定义结果文件路径 --->
    normal_summary_path = os.path.join(output_base_path, "normal_docvqa_analysis_summary.json")
    ...
    # <--- 新增: 创建已完成任务的集合以便快速检查 --->
    # DocVQA数据集中，一个唯一的任务由 (sample_name, question) 确定
    completed_tasks = set()
    ...
    for sample_name, data in docvqa_data.items():
    ...
    # <--- 修改: 调整主循环以支持中断继续 --->
    with tqdm(total=len(all_tasks_to_process), desc="Processing DocVQA samples") as pbar:
        ...
        for sample_name, data, question_data in all_tasks_to_process:
            ...
            try:
                # 获取当前样本的OCR信息
                ocr_info = None
                if USE_OCR_BASE and sample_name in docvqa_data:
                    ocr_texts = docvqa_data[sample_name]["ocr"]
