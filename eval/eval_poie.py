...
def evaluate_output(output_file, test_data_file):
    ...
    # 加载测试数据以获取答案
    answers = load_poie_test_data(test_data_file)
    ...
    for result in results:
        task_type = result.get("task_type", result.get("task", "unknown"))
        pred = result["prediction"]
        gt = result["ground_truth"]
...
def main():
    parser = argparse.ArgumentParser(description='评估POIE数据集的预测结果')
    parser.add_argument('--output_file', '-o', type=str, 
                        default=os.path.join(_REPO_ROOT, "eval/result/poie/output.json"),
                        help='预测结果文件路径')
    parser.add_argument('--test_data', '-t', type=str,
                        default=os.path.join(_REPO_ROOT, "dataset/POIE/poie_test.json"),
                        help='测试数据文件路径')
