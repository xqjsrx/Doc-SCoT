...
def evaluate_output(output_file, test_data_file):
    ...
    # 加载测试数据以获取答案
    answers = load_sroie_test_data(test_data_file)
...
def main():
    parser = argparse.ArgumentParser(description='评估SROIE数据集的预测结果')
    parser.add_argument('--output_file', '-o', type=str, 
                        default=os.path.join(_REPO_ROOT, "eval/result/sroie/output.json"),
                        help='预测结果文件路径')
    
    parser.add_argument('--test_data', '-t', type=str,
                        default=os.path.join(_REPO_ROOT, "dataset/SROIE2019/sroie_test.json"),
                        help='测试数据文件路径')
