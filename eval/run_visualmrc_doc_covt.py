...
def load_visualmrc_test_data(test_data_path):
    """
    加载VisualMRC测试数据，格式与LayTextLLM项目一致
    """
    print(f"正在加载VisualMRC测试数据: {test_data_path}")
...
def main():
    ...
    image_folder_path = os.environ.get(
        "EVAL_IMAGE_FOLDER", os.path.join(_REPO_ROOT, "dataset/VisualMRC/images"))

    output_base_path = os.environ.get(
        "EVAL_OUTPUT_BASE",
        os.path.join(_REPO_ROOT, "eval/result/visualmrc/"),
    )  # 输出根路径

    visualmrc_test_data_path = os.environ.get(
        "EVAL_DATA_PATH", os.path.join(_REPO_ROOT, "dataset/VisualMRC/visualmrc_test.json"))
    ...
    model = CoVTQwen3VLForConditionalGeneration.from_pretrained(
        ...
        # 旧 checkpoint 内 flow 分支为 384 维, R 方案后代码为 264 维;
        # flow 模块仅训练用, 忽略尺寸不匹配。不可与 device_map=auto 同用
    ...
    # <--- 新增: 定义结果文件路径 --->
    normal_summary_path = os.path.join(output_base_path, "normal_visualmrc_analysis_summary.json")
    ...
    # <--- 新增: 创建已完成任务的集合以便快速检查 --->
    # VisualMRC数据集中，一个唯一的任务由 (sample_name, question) 确定
    completed_tasks = set()
    ...
    for sample_name, data in visualmrc_data.items():
        for question_data in data["questions"]:
            all_tasks_to_process.append((sample_name, data, question_data))

    # 多卡分片: EVAL_NUM_SHARDS/EVAL_SHARD_ID 控制, 各分片用独立输出目录,
    # 完成后用 merge_eval_shards.py 合并计算最终指标
    _num_shards = int(os.environ.get("EVAL_NUM_SHARDS", "1"))
    _shard_id = int(os.environ.get("EVAL_SHARD_ID", "0"))
    if _num_shards > 1:
        all_tasks_to_process = all_tasks_to_process[_shard_id::_num_shards]
        print(f"分片 {_shard_id}/{_num_shards}: 本分片任务数 {len(all_tasks_to_process)}")
            
    # ======================================================================

    # <--- 修改: 调整主循环以支持中断继续 --->
    with tqdm(total=len(all_tasks_to_process), desc="Processing VisualMRC samples") as pbar:
        ...
        for sample_name, data, question_data in all_tasks_to_process:
            ...
            try:
                # 获取当前样本的OCR信息
                ocr_info = None
                if USE_OCR_BASE and sample_name in visualmrc_data:
                    ocr_texts = visualmrc_data[sample_name]["ocr"]
